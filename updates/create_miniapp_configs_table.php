...
class CreateMiniappConfigsTable extends Migration
{
    public function up(): void
    {
        Schema::create('miniapp_configs', function (Blueprint $table) {
            ...
            $table->text('app_secret')->nullable()->comment('AppSecret(加密存储)');
            $table->string('token', 255)->default('')->comment('消息推送Token');
            $table->string('encoding_aes_key', 255)->default('')->comment('消息加解密密钥');
            $table->tinyInteger('is_verified')->default(0)->comment('是否已验证: 0否 1是');
            $table->tinyInteger('is_enabled')->default(0)->comment('是否启用: 0否 1是');
            $table->timestamps();
        });
    }
